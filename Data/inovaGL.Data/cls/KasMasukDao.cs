...
namespace inovaGL.Data
{
    public class AdnKasMasukDao
    {
        ...
        public void Simpan(AdnKasMasuk o)
        {
            ...
            if (o.KdKM.Trim() == "")
            {
                o.KdKM = this.GetNoBukti(o.Tgl,o.ThAjar);
                o.KdJurnal = o.KdKM;
            }
        ...
        public DataTable GetByArgs(string sFilter)
        {
            ...
            string sql =
            " select ac_tkm.kd_tkm, tgl, dr, deskripsi, kd_jurnal, jmh "
            + " from " + NAMA_TABEL
            + " inner join "
            + " ( "
            + "     select kd_tkm, sum(debet) jmh "
            + "     from ac_tkm_dtl "
            + "     where debet>0 "
            + "     group by kd_tkm "
            + " ) dtl "
            + " on ac_tkm.kd_tkm = dtl.kd_tkm ";
        ...
        public string GetNoBukti(DateTime Tgl)
        {
            ...
            sql =
            "SELECT isnull(max(right(kd_tkm,3)),0) as kd  "
            + " FROM " + NAMA_TABEL
            + " where left(kd_tkm,7) ='BKM" + periode.Trim() + "'";

            cmd.CommandText = sql;
            int iMax = Convert.ToInt32(cmd.ExecuteScalar()) + 1;
            kode = "BKM" + periode + iMax.ToString("000");
        ...
        public string GetNoBukti(DateTime Tgl, string ThAjar)
        {
            ...
            sql =
            "SELECT isnull(max(right(kd_tkm,5)),0) as kd  "
            + " FROM " + NAMA_TABEL
            + " where left(kd_tkm,7) ='BKM" + periode.Trim() + "'";

            cmd.CommandText = sql;
            int iMax = Convert.ToInt32(cmd.ExecuteScalar()) + 1;
            kode = "BKM" + periode + iMax.ToString("00000");
